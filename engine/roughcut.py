# Adapted from leadgenman-video-skills by Manthan Patel (MIT) -- see THIRD_PARTY_NOTICES.md
"""
...
# CONFIGURATION
# ============================================================
SILENCE_MERGE_THRESHOLD = float(os.environ.get("RC_SILENCE_MERGE", 0.30))   # seconds -- tuned sweet spot for conversational speech
MIN_CHUNK_DURATION = float(os.environ.get("RC_MIN_CHUNK", 1.0))         # skip fragments under this
...
def produce(video_path):
    ...
    try:
        ...
        # ponytail: no OPENAI_API_KEY -> local whisper.cpp adapter (token-level
        # timestamps approximate word granularity; upgrade path is the real API)
        from local_whisper import LocalWhisper
        client = LocalWhisper()
        print("No OPENAI_API_KEY -- using local whisper-cli")
